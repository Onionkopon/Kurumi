...
from SungJinwooRobot import dispatcher, DRAGONS, DEMONS, LOGGER
from SungJinwooRobot.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import SungJinwooRobot.modules.helper_funcs.fun_msgs as dem


@run_async
@typing_action
def truth(update, context):
    update.effective_message.reply_text(random.choice(dem.TRUTH))

@run_async
@typing_action
def dare(update, context):
    update.effective_message.reply_text(random.choice(dem.DARE))
...
def runs(update, context):
    update.effective_message.reply_text(random.choice(dem.RUN_STRINGS))
...
def pat(update: Update, context: CallbackContext):
    ...
    if pat_type == "Gif":
        try:
            temp = random.choice(dem.PAT_GIFS)
    ...
    if pat_type == "Sticker":
        try:
            temp = random.choice(dem.PAT_STICKERS)
            reply_to.reply_sticker(temp)
        except BadRequest:
            pat_type = "Text"

    if pat_type == "Text":
        temp = random.choice(dem.PAT_TEMPLATES)
...
def slap(update, context):
    ...
    temp = random.choice(dem.SLAP_TEMPLATES)
    item = random.choice(dem.ITEMS)
    hit = random.choice(dem.HIT)
    throw = random.choice(dem.THROW)
...
def hug(update, context):
    ...
    temp = random.choice(dem.HUG_TEMPLATES)
    hug = random.choice(dem.HUG)
...
def abuse(update, context):
    ...
    reply_text(random.choice(dem.ABUSE_STRINGS))
...
def shrug(update, context):
    ...
    reply_text(random.choice(dem.SHGS))
...
def decide(update, context):
    ...
    if len(args) >= 2:  # Don't reply if no args
        ...
        reply_text(random.choice(dem.DECIDE))
...
def yesnowtf(update, context):
    ...
    if res.status_code != 200:
        return msg.reply_text(random.choice(dem.DECIDE))
    else:
        res = res.json()
    try:
...
def table(update, context):
    ...
    reply_text(random.choice(dem.TABLE))
...
def cri(update, context):
    ...
    reply_text(random.choice(dem.CRI))
...
def recite(update, context):
    ...
    reply_text(random.choice(dem.BEING_LOGICAL))
...
def gbun(update, context):
    ...
    if int(user.id) in DRAGONS or int(user.id) in DEMONS:
        context.bot.sendMessage(chat.id, (random.choice(dem.GBUN)))
...
def gbam(update, context):
    ...
    if int(user.id) in DRAGONS or int(user.id) in DEMONS:
        gbamm = dem.GBAM
        reason = random.choice(dem.GBAM_REASON)
...
✪ /meme*:* Sends random meme scraped from reddit
*Regex based memes:*
